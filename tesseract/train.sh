...
TROOT=`pwd`
mkdir $TROOT/stockfonts; mkdir $TROOT/build; mkdir $TROOT/build/por
echo "Environment built"

# Get the stock english fonts from Google (old, but they work)
cd $TROOT/stockfonts
GET http://www.k-type.com/freefonts/MANDATORY.zip > MANDATORY.zip
echo "Mandatory Font loaded"


# unpack the fonts, a new english (eng) directory is created with tif/box files
#tar -xzf boxtiff-2.01.eng.tar.gz
unzip -a MANDATORY.zip
echo "MANDATORY.zip file unpacked"


# Move the arial font data into the build space (yes, the exp0 is required)
mv $TROOT/stockfonts/por/por.mandatory.g4.tif $TROOT/build/eng.arial.exp0.tif
mv $TROOT/stockfonts/por/por.mandatory.box $TROOT/build/eng.arial.exp0.box
